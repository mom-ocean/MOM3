...
module mpp_io_mod
  ...
  implicit none
  private
  character(len=256), private :: version='$Id: mpp_io.F90,v 5.3 1999/12/03 16:59:31 vb Exp $' !RCS ID
...
!it's clunky and inelegant, but required so that axes can be shared among multiple files
  ...
  type, public  	       :: filemap_type
    sequence
    integer                    :: ncid
    integer                    :: unit
    integer                    :: nvars
    character(len=128)         :: filename
    type(fieldtype), pointer   :: ncvar(:)
  end type filemap_type
...
!null unit: returned by PEs not participating in IO after a collective call
  ...
  interface mpp_read
     ...
     module procedure mpp_read_4d
  end interface

  public :: mpp_close, mpp_flush, mpp_get_iospec, mpp_get_ncid, mpp_get_unit_range, mpp_io_init, mpp_io_exit, mpp_open, &
            mpp_set_unit_range, mpp_write, mpp_write_meta, mpp_read_meta, getvarid, getunlimid, mpp_read
...
!action: read, write, overwrite, append: act and pos are ignored by netCDF
      if( action_flag.EQ.MPP_RDONLY )then
          act = 'READ'
          pos = 'REWIND'
          if( form_flag.EQ.MPP_NETCDF )call mpp_error( WARNING, 'MPP_OPEN: reading with netCDF is under development.' )
...
!get the current time level of the file: writes to this file will be at next time level
	      ...
	      error = NF_OPEN(mpp_file(unit)%name,0,mpp_file(unit)%ncid); call netcdf_err(error)
              if( verbose ) print '(a,a,i3,i16)', 'MPP_OPEN: netCDF file: name, pe, ncid='&
	      , trim(mpp_file(unit)%name), pe, mpp_file(unit)%ncid
  	  end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!                                                                      !
!    mpp_read_meta: read the meta-information of a netcdf-file         !
!                                                                      !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    subroutine mpp_read_meta(unit,filemap,errorstop)
    integer            :: stout
    integer, intent(in):: unit
    type(filemap_type) :: filemap      
    logical,intent(in), optional :: errorstop
    logical            :: errstop
    integer            :: dimids(10)  ! id's of dimensions
    integer 	       :: ncid,ndims,nvars,id,ndim,time_axis_index
    integer , allocatable 		  :: len_dim(:)
    character(len=128), allocatable	  :: name_dim(:)
    character(len=80)             	  :: time_name,time_unit
    character(len=14), dimension(6)       :: xtype_name
...
!
    stout = pe + 6
    errstop = .false.
    if(present(errorstop)) errstop = errorstop
    ncid = mpp_get_ncid(unit)
    filemap%unit = unit
    filemap%filename = mpp_file(unit)%name
    filemap%ncid = ncid
    write(stout,*)'Reading initial conditions from file ' &
    		   ,trim(mpp_file(unit)%name),' opened with error = '&
                   ,error,' ncid is ',filemap%ncid
!
!-----------------------------------------------------------------------
...
!
    ndims = 0
    nvars = 0
    error=NF_INQ_NDIMS(ncid, ndims); call netcdf_err(error)
    error=NF_INQ_NVARS(ncid, nvars); call netcdf_err(error)
    write(stout,*)'ic file ',trim(mpp_file(unit)%name) &
           ,' has',ndims,' dimensions and',nvars,' variables'
!
!-----------------------------------------------------------------------
...
!
    ...
    do id=1,ndims
      error=NF_INQ_DIM(ncid, id, name_dim(id), len_dim(id)); call netcdf_err(error)
# ifdef debug_netcdf
      write(stout,*) 'Dimension(',id,'): ', trim(name_dim(id)) &
	             ,' Length: ', len_dim(id)
# endif
    enddo
!           
...
!
    filemap%nvars = nvars
    allocate(filemap%ncvar(1:nvars))
    do id=1,nvars
      error=NF_INQ_varname(ncid,id,filemap%ncvar(id)%name); call netcdf_err(error)
      error=NF_INQ_vartype(ncid,id,filemap%ncvar(id)%pack); call netcdf_err(error)
      filemap%ncvar(id)%units=' '
      error=NF_GET_ATT_text(ncid,id,'units',filemap%ncvar(id)%units)
      if(errstop)  call netcdf_err(error)
      filemap%ncvar(id)%longname=' '
      error=NF_GET_ATT_text(ncid,id,'long_name',filemap%ncvar(id)%longname)
      if(errstop)  call netcdf_err(error)
# ifdef debug_netcdf
      write(stout,*) 'Variable(',id,'): ', trim(filemap%ncvar(id)%name) &
        	      ,' ',xtype_name(filemap%ncvar(id)%pack) &
        	      ,' ',trim(filemap%ncvar(id)%units)
# endif
    enddo
!     
!-----------------------------------------------------------------------
...
!
    do id=1,nvars
!     get the number of axes for this variable
      error=NF_INQ_varndims(ncid,id,ndim); call netcdf_err(error)
      allocate (filemap%ncvar(id)%axes(ndim))
      allocate (filemap%ncvar(id)%size(ndim))
      error=NF_INQ_vardimid(ncid,id,dimids); call netcdf_err(error)
      filemap%ncvar(id)%axes(1:ndim)%id = dimids(1:ndim)
      time_axis_index = -1
      error=NF_INQ_unlimdim(ncid, time_axis_index) 
      filemap%ncvar(id)%time_axis_index = time_axis_index
      filemap%ncvar(id)%size(:) = len_dim(filemap%ncvar(id)%axes(:)%id)
# ifdef debug_netcdf
      write(stout,*) 'Variable(',id,'): ',trim(filemap%ncvar(id)%name) &
        ,' number of dimensions :', size(filemap%ncvar(id)%axes)
      write(stout,*) 'Variable(',id,'): ',trim(filemap%ncvar(id)%name) &
        ,' dimension id :', filemap%ncvar(id)%axes(:)%id
      write(stout,*) 'Variable(',id,'): ',trim(filemap%ncvar(id)%name) &
        ,' dimensions: ', filemap%ncvar(id)%size(:)
# endif
    enddo        
    if (allocated(name_dim)) deallocate(name_dim)
    if (allocated(len_dim))  deallocate(len_dim) 
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
    integer function getvarid(filemap,name)
    type(filemap_type)    :: filemap
    character(len=*) :: name
    integer          :: idvar, id
    integer          :: stout

    if(filemap%unit.eq.NULLUNIT) then
      return
    endif
    stout = pe + 6
    idvar = -1
    do id=1,size(filemap%ncvar)
      if (lower_case(name).eq.lower_case(filemap%ncvar(id)%name)) idvar=id
    enddo
    if (idvar.lt.1) then
      write(stout,*) 'Warning => variable ',trim(name),' not found in file'&
                    ,trim(filemap%filename)
#ifdef debug_netcdf
    else
      write(stout,*) 'get variable ',trim(name)&
  	 ,' from variable(',idvar,'): ',trim(filemap%ncvar(idvar)%name) &
  	 ,', units: ',trim(filemap%ncvar(idvar)%units)
#endif
    endif  
    getvarid = idvar 
    return
...
!-----------------------------------------------------------------------
    ...
    function lower_case(str_var)
    character (len=256) :: str1, lower_case
    character (len=*)   :: str_var
    integer             :: i2, i1, k
    str1 = ''
    do k=1, len_trim(str_var)
      i2=ichar(str_var(k:k))
      if(i2.ge.65.and.i2.le.90) i2=i2+32
      str1=trim(str1)//achar(i2)
    enddo
    lower_case = trim(str1)
    end function lower_case
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------

    integer function getunlimid(filemap,errorstop)
    type(filemap_type)    :: filemap
    logical, intent(in), optional :: errorstop
    integer          :: ncid, idunlim
    integer          :: stout
    logical          :: errstop
    ...
    if (present(errorstop)) errstop = errorstop
    ncid=filemap%ncid 
    error = nf_inq_unlimdim(ncid, idunlim) 
    if(errstop) then 
      call netcdf_err(error)
    else
      if (error.ne.nf_noerr) then
      idunlim = -1
      write(stout,*) 'Warning => No unlimited (time) axis found in file'&
                    ,trim(filemap%filename)
    endif  
#ifdef debug_netcdf
    write(stout,*) 'Variable ',trim(filemap%ncvar(idunlim)%name)&
  	 ,', units: ',trim(filemap%ncvar(idunlim)%units)&
  	 ,' is found as unlimited axis in file', trim(filemap%filename)
#endif
    endif  
    getunlimid = idunlim 
    return
    end function getunlimid
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------

    subroutine mpp_read_0d (filemap,varid,varname,varindex,value,errorstop)
! This subroutine reads a single value from a 1D-variable
    type(filemap_type)         :: filemap
    integer                    :: varindex   ! The index at the 1D-variable
    integer,          optional :: varid      ! The id of the 1D-variable
    character(len=*), optional :: varname    ! The 1D-var's name
    real                       :: value      ! The value to be returned
    integer                    :: stout
    logical, intent(in), optional :: errorstop
    logical                    :: errstop
    integer                    :: varid_temp
# ifdef debug_netcdf
    character(len=80)          :: var_name,var_unit
# endif
    ...
    errstop = .false.
    if (present(errorstop)) errstop = errorstop
    if ((.not.present(varid)).and.(.not.present(varname))) then
      write(stout,*) 'Error => mpp_read_0d! Neither varid nor varname is specified.'
      call abort()  
    endif
    ...
    if (present(varname)) then
      varid_temp = getvarid(filemap,varname)
    endif
    if (present(varid)) then
      if(varid.ne.varid_temp) then
        write(stout,*) 'Error => mpp_read_0D!',&
                        'varid = ',varid,' is specified!',&
	  	        'varid = ',varid_temp,' found in ',trim(filemap%filename)
        call abort()  
      endif
    endif
    if (kind(value).eq.4) then
      if (filemap%ncvar(varid_temp)%pack.eq.NF_DOUBLE) &
        write(stout,*) '=>Warning: reading a double variable to a buffer',&
                        ' of type float. This reduces accuracy!'
      error=NF_GET_VAR1_real(filemap%ncid,varid_temp,varindex,value);call netcdf_err(error)
    else
      error=NF_GET_VAR1_double(filemap%ncid,varid_temp,varindex,value);call netcdf_err(error)
    endif
#ifdef debug_netcdf
    var_name=' '
    var_unit=' '
    error=NF_GET_ATT_text(filemap%ncid,varid_temp,'long_name',var_name)
    if(errstop) call netcdf_err(error)
    error=NF_GET_ATT_text(filemap%ncid,varid_temp,'units',var_unit)
    if(errstop) call netcdf_err(error)
    write(stout,*) 'mpp_read_0d! Reading the variable '//trim(var_name)&
                   //'variable unit: ',trim(var_unit)
#endif
    return
    end subroutine mpp_read_0d

    subroutine mpp_read_1d (filemap,varid,varname,buff,errorstop)
! This subroutine reads a 1D-variable
    type(filemap_type)         :: filemap
    integer,          optional :: varid      ! The id of the axis in the file
    character(len=*), optional :: varname    ! The axis name
    real                       :: buff(:)    ! The data to be returned
    logical, intent(in), optional :: errorstop
    integer                    :: stout
...
# endif
    ...
    if (present(errorstop)) errstop = errorstop
    if ((.not.present(varid)).and.(.not.present(varname))) then
      write(stout,*) 'Error => mpp_read_1d! Neither varid nor varname is specified.'
      call abort()  
    endif
    if (present(varid)) then
      varid_temp = varid
    ...
    if (present(varid)) then
      if(varid.ne.varid_temp) then
        write(stout,*) 'Error => mpp_read_1d!',&
                        'varid = ',varid,' is specified!',&
	  	        'varid = ',varid_temp,' found in ',trim(filemap%filename)
        call abort()  
      endif
    endif
    if(size(filemap%ncvar(varid_temp)%axes).ne.1) then
      write(stout,*) 'Error => mpp_read_1D!',&
                      'Variable ',varid,' has more than one dimension!'
      call abort()  
    endif
    if (kind(buff).eq.4) then
      if (filemap%ncvar(varid_temp)%pack.eq.NF_DOUBLE) &
        write(stout,*) '=>Warning: reading a double variable to a buffer',&
                        ' of type float. This reduces accuracy!'
      error=nf_get_var_real(filemap%ncid,varid_temp,buff)
    else
      error=nf_get_var_double(filemap%ncid,varid_temp,buff)
    endif
    call netcdf_err(error)
#ifdef debug_netcdf
    var_name=' '
    ...
    error=NF_GET_ATT_text(filemap%ncid,varid_temp,'units',var_unit)
    if(errstop) call netcdf_err(error)
    write(stout,*) 'mpp_read_1d! Reading the variable '//trim(var_name)&
                   //'variable unit: ',trim(var_unit)
#endif
    return
    ...
    subroutine mpp_read_4d (filemap,varid,varname,start,count,buff,errorstop)
! This subroutine reads a 4D-variable (or a subset)
    type(filemap_type)         :: filemap
    integer,          optional :: varid      ! The id of the axis in the file
    character(len=*), optional :: varname    ! The axis name
    real                       :: buff(:,:,:,:) ! The value to be returned
    logical, intent(in), optional :: errorstop
    integer                    :: stout
    integer                    :: start(:), count(:)
    logical                    :: errstop
...
# endif
    ...
    if (present(errorstop)) errstop = errorstop
    if ((.not.present(varid)).and.(.not.present(varname))) then
      write(stout,*) 'Error => mpp_read_4d! Neither varid nor varname is specified.'
      call abort()  
    endif
    if (present(varid)) then
      varid_temp = varid
    ...
    if (kind(buff).eq.4) then
      if (filemap%ncvar(varid_temp)%pack.eq.NF_DOUBLE) &
        write(stout,*) '=>Warning: reading a double variable to a buffer',&
                        ' of type float. This reduces accuracy!'
      error=nf_get_vara_real(filemap%ncid,varid_temp,start,count,buff)
    else
      error=nf_get_vara_double(filemap%ncid,varid_temp,start,count,buff)
    endif
    call netcdf_err(error)
#ifdef debug_netcdf
    var_name=' '
    ...
    error=NF_GET_ATT_text(filemap%ncid,varid_temp,'units',var_unit)
    if(errstop) call netcdf_err(error)
    write(stout,*) 'mpp_read_4d! Reading the variable '//trim(var_name)&
                   //'variable unit: ',trim(var_unit)
#endif
    return
    end subroutine mpp_read_4d
