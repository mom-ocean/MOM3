...
!Balaji (vb@gfdl.gov) 15 March 1999
  ...
  implicit none
  private
  character(len=256), private :: version='$Id: mpp_domains.F90,v 5.0 1999/10/05 19:08:53 vb Exp $'
...
!some elements are repeated below if they are needed once per domain, not once per axis
  ...
  type(domain1D), target, private :: NULL_DOMAIN1D
  type(domain2D), target, private :: NULL_DOMAIN2D
...
!          call mpp_set_halo_size(32768) !default, pretty arbitrary
          call mpp_set_halo_size(40000) !default, pretty arbitrary
      end if
...
!USE WITH CARE! a global 3D array could occupy a lot of memory!
      ...
      call mpp_set_halo_size(domain%x%compute%max_size*domain%y%compute%max_size*size(local_field,3))
