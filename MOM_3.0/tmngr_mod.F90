...
type(time_type), public ::  model_time, ic_time, base_time
type(time_type), public ::  stop_time, time_since_base, dt_time
type(time_type), public ::  start_time
...
! set_switch=true when mod(time_since,switch_interval) <= dt_time/2
! author: R.C.Pacanowski         rcp@gfdl.gov 

implicit none

logical :: set_switch
real, intent(in) :: switch_interval             ! in units of days
type(time_type), intent(in) :: time_since, dt_time
type(time_type) :: interval_time, current_time, next_time, half_dt_time
integer :: n, seconds, days
...
!define some time structures
type(time_type) :: avg_time, mid_date, time_since_ic

integer :: num_steps, i, days, months, years, seconds, minutes, hours
integer :: months2, years2, length, num_years, itt, isec, iday
integer :: icyear, icmonth, icday, ichour, icmin, icsec
integer :: rfyear, rfmonth, rfday, rfhour, rfmin, rfsec
real :: astro_days, dtts, snapint, xbtint, xbtper, relday, relyr

stdout = 6
write (stdout,'(/,20x,a,/)') 'Driver for time manager module'
...
iday = 0      !days since initial condition date
isec = 0*1000 !converting to milli-sec
runlen0 = 365.0 !days to integrate
runlen0 = 100.0 !days to integrate
refuser = .false. !reference switch intervals to a user specified time
refinit = .true.  !reference switch intervals to Initial condition time
refrun  = .false. !reference switch intervals to time at start of run

call tmngri (icyear, icmonth, icday, ichour, icmin, icsec, &
             rfyear, rfmonth, rfday, rfhour, rfmin, rfsec, &
             iday, isec, &
             runlen0, 'days', rundays0, dtts)

! Set some diagnostic intervals (units are in days. see switch.F)
snapint = 10.0
xbtint  = 25.0
xbtper  = 0.5
...
! Example: Increment model_time and time since initial conditions by "dtts" seconds
  call tmngr (dtts)

! Example: show some time steps and corresponding times
  if (itt .lt. 26) then
    call get_date(model_time, years, months, days, hours, minutes, seconds)
    call set_stamp (stamp, years, months, days, hours, minutes, seconds)
    call get_time (model_time - ic_time, seconds, days)
    relday = days + seconds/86400.
    relyr  = relday/365.25
    write (stdout,'(a,i10,a,a,a,1pe14.7,a,e14.7)') ' itt=',itt &
     ,', time stamp=',stamp &
     ,', days since IC=',relday,', years since IC=',relyr
  endif

  call set_time_switches !switches are set based on model_time

  if (itt .lt. 26) then
  write (stdout,*) 'first=',first
  endif
! Example: test logical switch for periodic snapshot of data
  if (snapts) then
      call get_date(model_time, years, months, days, hours, minutes, seconds)
      call set_stamp (stamp, years, months, days, hours, minutes, seconds)
      call get_time(time_since_base, seconds, days)
      relday = days + seconds / 86400.
      write(stdout, '(a,i10,a,a,a,f12.4,a,a)') &
       '->Snapshot at: itt=',itt, ' ',stamp,' which is ',relday,' days since base'
   endif

! Example: test logical switches for averaging within a periodic interval
  if (xbtperts) then
    call get_date(model_time, years, months, days, hours, minutes, seconds)
    call set_stamp (stamp, years, months, days, hours, minutes, seconds)
    call get_time(time_since_base, seconds, days)
    relday = days + seconds / 86400.
    write(stdout, '(a,i10,a,a,a,f12.4,a,a)') &
    '->Accumulating xbt data: itt=',itt, ' ',stamp,' which is ',relday,' days since base'
  endif
  if (xbtts) then
    call get_date(model_time, years, months, days, hours, minutes, seconds)
    call set_stamp (stamp, years, months, days, hours, minutes, seconds)
    ...
    relday = days + seconds / 86400.
    write(stdout, '(a,i10,a,a,a,f12.4,a,a)') &
     '=>xbt average: itt=',itt, ' ',stamp,' which is ',relday,' days since base'
  endif

! Example: test logical switch for mid-month
  if (midmon) then
    write(stdout, *) '--->middle of month: itt=',itt, stamp
  endif

! Example: test logical switch for the end of the month 
  if (eomon) then
    write(stdout, *) '-->last timestep of month: itt=',itt, ' ',stamp
  endif

! Example: test logical switch for the end of the year
  if (eoyear) then
    write(stdout, *) '-->last timestep of year : itt=',itt, ' ',stamp
  endif

  if (eorun) then
    call get_date(model_time, years, months, days, hours, minutes, seconds)
    call set_stamp (stamp, years, months, days, hours, minutes, seconds)
    write(stdout, *) '--->End of run on itt=',itt,' ', stamp
    stop
  endif
