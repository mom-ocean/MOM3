...
#if defined(__sgi) && defined(use_libSMA)
  use shmem_interface
#endif
  implicit none
  private
  character(len=256), private :: version='$Id: mpp.F90,v 5.1 1999/10/26 15:48:17 vb Exp $'
...
#include <mpp/shmem.fh>
  ...
  integer(LONG_KIND), private :: remote_data_ptr !used to send pointer to remote data
#ifdef SGICRAY_MPP
...
!tag is never used, but must be initialized to non-negative integer
  integer, private :: tag=1, request=0, stat(MPI_STATUS_SIZE), group_all
#ifdef _CRAYT3E
...
#endif
  public :: mpp_error, mpp_exit, mpp_init, mpp_max, mpp_min, mpp_npes, mpp_pe, mpp_recv, mpp_send, &
...
!we use shpalloc to ensure all these are remotely accessible
      ...
      call SHMEM_BARRIER_ALL()
      call SHPALLOC( ptr$remote_data_ptr, WORDS_PER_REAL, error, -1 )
      call SHMEM_BARRIER_ALL()
...
!we just wait for it to be .NE.MPP_WAIT_VALUE
          ...
          call SHMEM_PUT8( remote_data_ptr, data_loc, 1, put_pe )
          call SHMEM_INTEGER_PUT( mpp_get_pe, pe, 1, put_pe )
#endif use_libSMA
#ifdef use_libMPI
!use non-blocking sends
          call MPI_ISEND( put_data, put_len, MPI_REAL8, put_pe, tag, MPI_COMM_WORLD, request, error )
#endif
...
!do the get: for libSMA, a get means do a wait to ensure put on remote PE is complete
      if( get_pe.GE.0 .AND. get_pe.LT.npes )then
          ...
          call SHMEM_UDCFLUSH !invalidate data cache
#endif
          call SHMEM_INT8_WAIT( remote_data_ptr, MPP_WAIT_VALUE )
          ptr$remote_data = remote_data_ptr
          remote_data_ptr = MPP_WAIT_VALUE !reset
#if defined(CRAYPVP) || defined(__sgi)
...
!this is only required for !CRAYPVP  && !__sgi, but is done there too, so that we can send put_is_done back.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!                                                                              !
!       GLOBAL REDUCTION ROUTINES: mpp_max, mpp_sum                            !
!                                                                              !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!sum on pe 0 and broadcast
      ...
      call MPI_REDUCE( a, a, 1, MPI_REAL8, MPI_MAX, 0, peset(1), error )
...
!sum on pe 0 and broadcast
      if( verbose )call mpp_error( NOTE, 'MPP_MAX: using MPI_REDUCE and MPI_BCAST...' )
      call MPI_REDUCE( a, a, 1, MPI_INTEGER8, MPI_MAX, 0, peset(1), error )
...
!sum on pe 0 and broadcast
      ...
      call MPI_REDUCE( a, a, 1, MPI_REAL8, MPI_MIN, 0, peset(1), error )
      call MPI_BCAST( a, 1, MPI_REAL8, 0, peset(1), error )
#endif
      return
...
!sum on pe 0 and broadcast
      if( verbose )call mpp_error( NOTE, 'MPP_MIN: using MPI_REDUCE and MPI_BCAST...' )
      call MPI_REDUCE( a, a, 1, MPI_INTEGER8, MPI_MIN, 0, peset(1), error )
      call MPI_BCAST( a, 1, MPI_INTEGER8, 0, peset(1), error )
#endif
      return
...
!sum on pe 0 and broadcast
      ...
      call MPI_REDUCE( a, a, length, MPI_REAL8, MPI_SUM, 0, peset(1), error )
...
!synchronize PEs in list
      ...
      remote_data_ptr = MPP_WAIT_VALUE
      put_is_done = pe
      mpp_get_pe = ANY_PE
      call SHMEM_BARRIER_ALL()
#endif
#ifdef use_libMPI
!currently no barriers required on MPI, I believe
...
#endif use_libSMA
#ifdef use_libMPI
      call MPI_WAIT( request, stat, error )
#endif
      put_is_done = pe
      return
