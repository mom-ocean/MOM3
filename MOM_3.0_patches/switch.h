...
!
!     snapint_reg 
!             = number of days between writing instantaneous samples
!               of data from regions. note: only when "snapshots_regions"
!               is enabled. see "iounit.h" for more details.
!               all data is at time level "tau" so it is one time step
!               before the date shown in "stamp".
!
...
!
!     snapts_reg  
!             = (false,true) = (don`t, do) save a snapshot of the data
!               in regions on this time step. based on "snapint_reg"
!
...
# include "diag_region_out.h"
      logical          snapts_reg(nregions)   
      common /switcr/  snapint_reg(nregions)  
      common /switcl/  snapts_reg   
#endif
!   
!-----------------------------------------------------------------------
!
