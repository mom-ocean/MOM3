...
module mpp_io_mod
  ...
  implicit none
  private
  character(len=256), private :: version='$Id: mpp_io.F90,v 5.1 1999/10/26 15:48:52 vb Exp $' !RCS ID
...
#ifdef _CRAYT3E
              call ASSIGN( 'assign -I -F global.privpos f:'//trim(mpp_file(unit)%name) )
#endif
...
#ifdef CRAYPVP
      call ASSIGN( 'assign -I -P thread  f:'//trim(mpp_file(unit)%name) )
#endif
#ifdef _CRAYT3E
      call ASSIGN( 'assign -I -P private f:'//trim(mpp_file(unit)%name) )
#endif
...
#ifdef SGICRAY
          call ASSIGN( 'assign -I '//trim(iospec)//' f:'//trim(mpp_file(unit)%name) )
          if( form_flag.EQ.MPP_NETCDF )then
...
#ifdef _CRAY
              call ASSIGN( 'assign -I -N ieee_32 f:'//trim(mpp_file(unit)%name) )
#endif
...
#ifdef SGICRAY
      call ASSIGN( 'assign -R f:'//trim(mpp_file(unit)%name) )
#endif
      mpp_file(unit)%name = ' '
...
!currently will write to stdout: don't know how to trap and return as string to iospec
      call ASSIGN( 'assign -V f:'//trim(mpp_file(unit)%name) )
#endif
      return
